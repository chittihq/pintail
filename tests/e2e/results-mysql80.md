...
Measured 2026-09-05T21:22:59.574Z.
...
| activity-history | activity-history:the history is in the control plane pintail reads | PASS | 150037 sync_runs rows for db_afba9183a18c49845e2f38827dd98132 |
...
| activity-history | activity-history:25 concurrent feed reads do not pile up | PASS | p50 25ms p99 45ms |
...
| poll-storm | poll-storm:no request fails under 25 open dashboards | PASS | 0 failed of 4729 |
| poll-storm | poll-storm:latency stays bounded | PASS | 4729 requests: p50 3ms p99 24ms |
| poll-storm | poll-storm:health never stalls | PASS | health p99 9ms |
| poll-storm | poll-storm:replication keeps pace under the storm | PASS | orders replica 828 vs source 828 |
...
| drop-table-polling | polling:one dropped table does not stop the other tables | WARN | the whole poll cycle aborts on the first table that fails, so every other table stops replicating too: {"database":{"id":"db_afba9183a18c49845e2f38827dd98132","name":"e2e_db","mode":"polling","effective_mode":"polling","state":"error","include_tables":[],"exclude_tables":[],"poll_interval_seconds":5,"reconcile_interval_seconds":600,"keyless_policy":"quarantine","created_at":"2026-09-05T21:13:07.782016+00:00","updated_at":"2026-09-05T21:20:53.203317+00:00"},"tables":18,"rows":1336} |
...
| memory-pressure | memory-pressure:the process survives the storm | PASS | wire 240 ok, http 66 ok, dashboards 180 ok; no errors |
| memory-pressure | memory-pressure:every failure is a designed refusal | PASS | only refusals; 0 dashboard requests failed |
| memory-pressure | memory-pressure:work still gets done | PASS | wire 240 of 240, http 66 |
| memory-pressure | memory-pressure:wire queries are not starved by the HTTP surface | PASS | wire p50 424ms p99 1453ms over 240 queries |
| memory-pressure | memory-pressure:health never stalls | PASS | health p99 8ms over 13 samples |
| memory-pressure | memory-pressure:the process stays inside its ceiling | PASS | peak RSS 267MB with a 256MB budget |
...
| reconcile-memory | reconcile-memory:reconciliation converges the replica on the source | PASS | child 1800000 vs source 1800000 after 13.6s |
| reconcile-memory | reconcile-memory:reconciliation is bounded in memory | PASS | RSS 31MB before, peak 177MB during (margin 768MB) |
...
|---|---|---|---|
| snapshot | 0.0 | 1.1 | 1.0 |
| orm-compat | 9.3 | 0.4 | 0.7 |
| crud | 0.4 | 2.6 | 0.7 |
| type-edges | 0.0 | 1.3 | 0.8 |
| ddl | 10.0 | 15.7 | 0.7 |
| schema-drift-minimal | 0.2 | 5.2 | 0.9 |
| schema-drift-unseen | 0.9 | 5.4 | 0.5 |
| churn | 11.6 | 0.8 | 0.7 |
| contention | 14.1 | 0.8 | 0.7 |
| execution-budget | 0.0 | 0.5 | 0.5 |
| spill | 3.1 | 0.5 | 0.8 |
| pooling | 0.1 | 1.0 | 0.8 |
| local-database | 0.1 | 0.6 | 0.9 |
| restart | 0.6 | 3.0 | 1.1 |
| activity-history | 1.6 | 0.3 | 0.9 |
| poll-storm | 31.2 | 0.5 | 0.7 |
| control-plane | 53.5 | 1.0 | 1.1 |
| snapshot-ddl-window | 6.8 | 1.4 | 1.2 |
| drop-table-cdc | 21.2 | 1.3 | 0.7 |
| drop-table-recreate | 129.5 | 0.8 | 0.7 |
| drop-table-polling | 123.0 | 1.1 | 0.8 |
| restart-during-snapshot | 5.6 | 1.1 | 0.5 |
| restart-during-resync | 11.0 | 0.9 | 0.7 |
| memory-pressure | 15.8 | 1.6 | 0.7 |
| reconcile-memory | 51.6 | 1.1 | 1.0 |
| drop-database | 13.3 | 0.7 | 0.8 |
| ddl-documented-gaps | 0.1 | 1.2 | 0.7 |
| total | 514.5 | 51.8 | 21.1 |
