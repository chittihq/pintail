...
Measured 2026-09-24T08:13:29.288Z.
...
| activity-history | activity-history:the history is in the control plane pintail reads | PASS | 150030 sync_runs rows for db_2a07a1d06d42eb20b1d569bb32db6a42 |
| activity-history | activity-history:the feed pages the full history | PASS | limit=200 returned 200 |
| activity-history | activity-history:scoped feed stays fast over a large history | PASS | p50 2ms p95 3ms over 150000 rows |
...
| poll-storm | poll-storm:no request fails under 25 open dashboards | PASS | 0 failed of 4932 |
| poll-storm | poll-storm:latency stays bounded | PASS | 4932 requests: p50 1ms p99 10ms |
| poll-storm | poll-storm:health never stalls | PASS | health p99 2ms |
| poll-storm | poll-storm:replication keeps pace under the storm | PASS | orders replica 7017 vs source 7017 |
...
| drop-table-recreate | recreate:a table recreated under the same name replicates as a new table | PASS |  |
...
| drop-table-polling | polling:one dropped table does not stop the other tables | WARN | the whole poll cycle aborts on the first table that fails, so every other table stops replicating too: {"database":{"restored_backup_created_at":null,"data_age_seconds":null,"id":"db_2a07a1d06d42eb20b1d569bb32db6a42","name":"e2e_db","mode":"polling","effective_mode":"polling","state":"error","include_tables":[],"exclude_tables":[],"poll_interval_seconds":5,"reconcile_interval_seconds":600,"keyless_policy":"quarantine","created_at":"2026-09-24T08:08:11.177564816+00:00","updated_at":"2026-09-24T08:12:18.239196399+00:00"},"tables":30,"rows":10930} |
...
| memory-pressure | memory-pressure:the process survives the storm | PASS | wire 240 ok, http 94 ok, dashboards 144 ok; no errors |
| memory-pressure | memory-pressure:every failure is a designed refusal | PASS | only refusals; 0 dashboard requests failed |
| memory-pressure | memory-pressure:work still gets done | PASS | wire 240 of 240, http 94 |
| memory-pressure | memory-pressure:wire queries are not starved by the HTTP surface | PASS | wire p50 251ms p99 1606ms over 240 queries |
| memory-pressure | memory-pressure:health never stalls | PASS | health p99 19ms over 10 samples |
| memory-pressure | memory-pressure:the process stays inside its ceiling | PASS | peak RSS 145MB with a 256MB budget |
...
| reconcile-memory | reconcile-memory:reconciliation is bounded in memory | PASS | RSS 62MB before, peak 226MB during (margin 768MB) |
