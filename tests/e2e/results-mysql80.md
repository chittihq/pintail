...
Measured 2026-09-11T16:16:44.235Z.
...
| activity-history | activity-history:the history is in the control plane pintail reads | PASS | 150048 sync_runs rows for db_b4ffa6e0eead98f4e9ed3a6ee620387c |
| activity-history | activity-history:the feed pages the full history | PASS | limit=200 returned 200 |
| activity-history | activity-history:scoped feed stays fast over a large history | PASS | p50 5ms p95 10ms over 150000 rows |
| activity-history | activity-history:workspace feed stays fast over a large history | PASS | p50 6ms p95 13ms |
| activity-history | activity-history:25 concurrent feed reads do not pile up | PASS | p50 35ms p99 54ms |
...
| poll-storm | poll-storm:no request fails under 25 open dashboards | PASS | 0 failed of 4598 |
| poll-storm | poll-storm:latency stays bounded | PASS | 4598 requests: p50 6ms p99 33ms |
| poll-storm | poll-storm:health never stalls | PASS | health p99 10ms |
| poll-storm | poll-storm:replication keeps pace under the storm | PASS | orders replica 4451 vs source 4451 |
...
| control-plane | api:a paused table skips its changes and is recopied on resume | PASS |  |
...
| drop-table-polling | polling:one dropped table does not stop the other tables | WARN | the whole poll cycle aborts on the first table that fails, so every other table stops replicating too: {"database":{"restored_backup_created_at":null,"data_age_seconds":null,"id":"db_b4ffa6e0eead98f4e9ed3a6ee620387c","name":"e2e_db","mode":"polling","effective_mode":"polling","state":"error","include_tables":[],"exclude_tables":[],"poll_interval_seconds":5,"reconcile_interval_seconds":600,"keyless_policy":"quarantine","created_at":"2026-09-11T16:08:36.334847097+00:00","updated_at":"2026-09-11T16:15:35.469072295+00:00"},"tables":30,"rows":8364} |
...
| memory-pressure | memory-pressure:the process survives the storm | PASS | wire 240 ok, http 86 ok, dashboards 73 ok; no errors |
| memory-pressure | memory-pressure:every failure is a designed refusal | PASS | only refusals; 0 dashboard requests failed |
| memory-pressure | memory-pressure:work still gets done | PASS | wire 240 of 240, http 86 |
| memory-pressure | memory-pressure:wire queries are not starved by the HTTP surface | PASS | wire p50 134ms p99 629ms over 240 queries |
| memory-pressure | memory-pressure:health never stalls | PASS | health p99 15ms over 5 samples |
...
| reconcile-memory | reconcile-memory:reconciliation is bounded in memory | PASS | RSS 60MB before, peak 200MB during (margin 768MB) |
...
|---|---|---|---|
| snapshot | 0.0 | 0.3 | 1.4 |
| orm-compat | 6.2 | 0.3 | 1.1 |
| crud | 0.0 | 1.1 | 4.3 |
| composite-keys | 0.0 | 0.5 | 3.6 |
| type-edges | 0.0 | 1.5 | 3.4 |
| ddl | 7.0 | 3.5 | 2.9 |
| schema-drift-minimal | 0.0 | 1.0 | 4.2 |
| schema-drift-unseen | 0.0 | 0.9 | 3.0 |
| churn | 0.7 | 0.9 | 2.6 |
| contention | 13.7 | 0.2 | 4.7 |
| execution-budget | 0.0 | 0.2 | 4.6 |
| spill | 6.6 | 0.4 | 3.7 |
| pooling | 0.2 | 0.5 | 3.7 |
| local-database | 0.1 | 0.2 | 4.1 |
| restart | 0.6 | 2.6 | 5.6 |
| activity-history | 1.7 | 0.5 | 6.1 |
| poll-storm | 23.2 | 0.2 | 4.8 |
| control-plane | 32.7 | 0.1 | 0.8 |
| snapshot-ddl-window | 5.1 | 0.1 | 0.8 |
| drop-table-cdc | 9.9 | 0.1 | 1.1 |
| drop-table-recreate | 128.8 | 0.1 | 1.1 |
| drop-table-polling | 103.8 | 0.1 | 0.8 |
| restart-during-snapshot | 2.3 | 0.1 | 0.8 |
| restart-during-resync | 3.8 | 0.1 | 0.8 |
| memory-pressure | 8.4 | 0.1 | 0.8 |
| reconcile-memory | 32.1 | 0.1 | 0.8 |
| drop-database | 10.8 | 0.1 | 0.8 |
| ddl-documented-gaps | 0.0 | 0.1 | 0.8 |
| total | 397.9 | 15.7 | 73.2 |
