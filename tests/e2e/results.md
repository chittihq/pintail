...
Measured 2026-09-11T08:13:43.605Z.
...
| activity-history | activity-history:the history is in the control plane pintail reads | PASS | 150034 sync_runs rows for db_e65a39d9ebf04abb51a06c8dded4e4d5 |
...
| activity-history | activity-history:25 concurrent feed reads do not pile up | PASS | p50 41ms p99 140ms |
| activity-history | activity-history:health answers while the feed is hammered | PASS | health p95 6ms |
...
| poll-storm | poll-storm:no request fails under 25 open dashboards | PASS | 0 failed of 4933 |
| poll-storm | poll-storm:latency stays bounded | PASS | 4933 requests: p50 1ms p99 10ms |
| poll-storm | poll-storm:health never stalls | PASS | health p99 2ms |
| poll-storm | poll-storm:replication keeps pace under the storm | PASS | orders replica 7479 vs source 7479 |
...
| drop-table-polling | polling:one dropped table does not stop the other tables | WARN | the whole poll cycle aborts on the first table that fails, so every other table stops replicating too: {"database":{"restored_backup_created_at":null,"data_age_seconds":null,"id":"db_e65a39d9ebf04abb51a06c8dded4e4d5","name":"e2e_db","mode":"polling","effective_mode":"polling","state":"error","include_tables":[],"exclude_tables":[],"poll_interval_seconds":5,"reconcile_interval_seconds":600,"keyless_policy":"quarantine","created_at":"2026-09-11T08:06:25.297480797+00:00","updated_at":"2026-09-11T08:12:42.797117581+00:00"},"tables":30,"rows":11392} |
...
| memory-pressure | memory-pressure:the process survives the storm | PASS | wire 240 ok, http 79 ok, dashboards 102 ok; no errors |
| memory-pressure | memory-pressure:every failure is a designed refusal | PASS | only refusals; 0 dashboard requests failed |
| memory-pressure | memory-pressure:work still gets done | PASS | wire 240 of 240, http 79 |
| memory-pressure | memory-pressure:wire queries are not starved by the HTTP surface | PASS | wire p50 192ms p99 861ms over 240 queries |
| memory-pressure | memory-pressure:health never stalls | PASS | health p99 17ms over 7 samples |
| memory-pressure | memory-pressure:the process stays inside its ceiling | PASS | peak RSS 184MB with a 256MB budget |
...
| reconcile-memory | reconcile-memory:reconciliation is bounded in memory | PASS | RSS 63MB before, peak 214MB during (margin 768MB) |
...
|---|---|---|---|
| snapshot | 0.0 | 0.1 | 0.8 |
| orm-compat | 2.3 | 0.0 | 0.8 |
| crud | 0.0 | 2.0 | 0.9 |
| composite-keys | 0.0 | 1.6 | 0.9 |
| type-edges | 0.0 | 1.6 | 0.9 |
| ddl | 6.6 | 2.7 | 0.8 |
| schema-drift-minimal | 0.0 | 1.6 | 0.8 |
| schema-drift-unseen | 0.0 | 1.6 | 0.7 |
| churn | 0.2 | 1.6 | 0.8 |
| contention | 13.4 | 0.1 | 2.4 |
| execution-budget | 0.0 | 0.1 | 2.8 |
| spill | 6.5 | 0.5 | 1.5 |
| pooling | 0.9 | 0.1 | 1.5 |
| local-database | 0.0 | 1.3 | 2.3 |
| restart | 0.5 | 3.3 | 2.3 |
| activity-history | 1.1 | 0.5 | 2.2 |
| poll-storm | 22.7 | 0.1 | 2.3 |
| control-plane | 32.4 | 0.1 | 1.0 |
| snapshot-ddl-window | 5.1 | 0.1 | 1.0 |
| drop-table-cdc | 9.4 | 0.1 | 1.3 |
| drop-table-recreate | 128.6 | 0.2 | 1.5 |
| drop-table-polling | 103.9 | 0.1 | 0.9 |
| restart-during-snapshot | 1.6 | 0.1 | 0.9 |
| restart-during-resync | 3.6 | 0.1 | 0.9 |
| memory-pressure | 10.0 | 0.1 | 0.9 |
| reconcile-memory | 21.2 | 0.1 | 0.9 |
| drop-database | 11.6 | 0.1 | 0.9 |
| ddl-documented-gaps | 0.0 | 0.1 | 0.8 |
| total | 381.7 | 20.0 | 35.5 |
