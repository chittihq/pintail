...
Measured 2026-09-11T03:36:25.374Z.
...
| activity-history | activity-history:the history is in the control plane pintail reads | PASS | 150031 sync_runs rows for db_8eac1e41d5a69fce44ba8052486db201 |
...
| activity-history | activity-history:25 concurrent feed reads do not pile up | PASS | p50 25ms p99 30ms |
...
| poll-storm | poll-storm:no request fails under 25 open dashboards | PASS | 0 failed of 4927 |
| poll-storm | poll-storm:latency stays bounded | PASS | 4927 requests: p50 1ms p99 10ms |
| poll-storm | poll-storm:health never stalls | PASS | health p99 1ms |
| poll-storm | poll-storm:replication keeps pace under the storm | PASS | orders replica 7564 vs source 7564 |
...
| drop-table-polling | polling:one dropped table does not stop the other tables | WARN | the whole poll cycle aborts on the first table that fails, so every other table stops replicating too: {"database":{"restored_backup_created_at":null,"data_age_seconds":null,"id":"db_8eac1e41d5a69fce44ba8052486db201","name":"e2e_db","mode":"polling","effective_mode":"polling","state":"error","include_tables":[],"exclude_tables":[],"poll_interval_seconds":5,"reconcile_interval_seconds":600,"keyless_policy":"quarantine","created_at":"2026-09-11T03:29:16.383168908+00:00","updated_at":"2026-09-11T03:35:25.155681741+00:00"},"tables":30,"rows":11477} |
...
| memory-pressure | memory-pressure:the process survives the storm | PASS | wire 240 ok, http 81 ok, dashboards 90 ok; no errors |
| memory-pressure | memory-pressure:every failure is a designed refusal | PASS | only refusals; 0 dashboard requests failed |
| memory-pressure | memory-pressure:work still gets done | PASS | wire 240 of 240, http 81 |
| memory-pressure | memory-pressure:wire queries are not starved by the HTTP surface | PASS | wire p50 166ms p99 828ms over 240 queries |
| memory-pressure | memory-pressure:health never stalls | PASS | health p99 15ms over 7 samples |
| memory-pressure | memory-pressure:the process stays inside its ceiling | PASS | peak RSS 176MB with a 256MB budget |
...
| reconcile-memory | reconcile-memory:reconciliation is bounded in memory | PASS | RSS 63MB before, peak 218MB during (margin 768MB) |
