...
Measured 2026-09-10T06:56:31.925Z.
...
| activity-history | activity-history:the history is in the control plane pintail reads | PASS | 150031 sync_runs rows for db_24ca29a809fe3a6bb9bfd204f82e9ad4 |
...
| activity-history | activity-history:25 concurrent feed reads do not pile up | PASS | p50 36ms p99 42ms |
...
| poll-storm | poll-storm:latency stays bounded | PASS | 4913 requests: p50 1ms p99 10ms |
| poll-storm | poll-storm:health never stalls | PASS | health p99 2ms |
| poll-storm | poll-storm:replication keeps pace under the storm | PASS | orders replica 5149 vs source 5149 |
...
| drop-table-polling | polling:one dropped table does not stop the other tables | WARN | the whole poll cycle aborts on the first table that fails, so every other table stops replicating too: {"database":{"restored_backup_created_at":null,"data_age_seconds":null,"id":"db_24ca29a809fe3a6bb9bfd204f82e9ad4","name":"e2e_db","mode":"polling","effective_mode":"polling","state":"error","include_tables":[],"exclude_tables":[],"poll_interval_seconds":5,"reconcile_interval_seconds":600,"keyless_policy":"quarantine","created_at":"2026-09-10T06:49:35.759882915+00:00","updated_at":"2026-09-10T06:55:37.769058682+00:00"},"tables":21,"rows":7874} |
...
| memory-pressure | memory-pressure:the process survives the storm | PASS | wire 240 ok, http 85 ok, dashboards 96 ok; no errors |
...
| memory-pressure | memory-pressure:wire queries are not starved by the HTTP surface | PASS | wire p50 186ms p99 801ms over 240 queries |
| memory-pressure | memory-pressure:health never stalls | PASS | health p99 16ms over 7 samples |
| memory-pressure | memory-pressure:the process stays inside its ceiling | PASS | peak RSS 183MB with a 256MB budget |
...
| reconcile-memory | reconcile-memory:reconciliation is bounded in memory | PASS | RSS 55MB before, peak 126MB during (margin 768MB) |
...
| orm-compat | 4.4 | 0.1 | 0.4 |
| crud | 0.0 | 0.4 | 0.6 |
| composite-keys | 0.0 | 2.0 | 0.2 |
| type-edges | 0.0 | 2.4 | 0.3 |
| ddl | 7.2 | 2.8 | 0.3 |
| schema-drift-minimal | 0.0 | 1.9 | 0.7 |
| schema-drift-unseen | 0.1 | 2.2 | 0.8 |
| churn | 0.6 | 0.6 | 0.7 |
| contention | 13.8 | 0.2 | 1.8 |
| execution-budget | 0.0 | 0.2 | 1.7 |
| spill | 6.5 | 0.3 | 0.6 |
| pooling | 0.1 | 1.3 | 0.7 |
| local-database | 0.0 | 0.1 | 0.6 |
| restart | 0.5 | 2.4 | 0.6 |
| activity-history | 1.0 | 0.3 | 0.7 |
| poll-storm | 21.4 | 0.1 | 0.6 |
| control-plane | 32.2 | 0.1 | 0.4 |
| snapshot-ddl-window | 5.0 | 0.1 | 0.5 |
| drop-table-cdc | 8.4 | 0.1 | 0.8 |
| drop-table-recreate | 129.1 | 0.1 | 0.6 |
| drop-table-polling | 104.4 | 0.1 | 0.3 |
| restart-during-snapshot | 1.6 | 0.1 | 0.3 |
| restart-during-resync | 3.6 | 0.1 | 0.3 |
| memory-pressure | 9.6 | 0.1 | 0.3 |
| reconcile-memory | 18.7 | 0.1 | 0.3 |
| drop-database | 12.5 | 0.1 | 0.3 |
| ddl-documented-gaps | 0.0 | 0.1 | 0.3 |
| total | 380.8 | 18.2 | 16.1 |
