...
fn schema() -> TableSchema {
    TableSchema::new(
        ...
        vec![
            ...
            Column::new(2, "bucket", DataType::UInt64, false),
            Column::new(3, "metric", DataType::Int64, true),
...
fn make_row(id: usize, version: u64, metric: i64, note: &str, deleted: bool) -> StoredRow {
    StoredRow::new(
        PrimaryKey::new(vec![KeyPart::UInt64(id as u64)]).unwrap(),
        vec![
            Value::UInt64(id as u64),
            Value::UInt64(id as u64 % 64),
            Value::Int64(metric),
...
impl Fixture {
    fn new(n: usize) -> Self {
        ...
        .unwrap();
        let rows: Vec<_> = (0..n)
            .map(|i| make_row(i, i as u64 + 1, (i % 1000) as i64, "initial", false))
            .collect();
        ...
        let entry = TableEntry::new(
            ...
            "samples",
            schema(),
            TableStatistics::with_row_count(n as u64),
        )
        .unwrap();
        let catalog =
            CatalogSnapshot::new([DatabaseEntry::new(DatabaseId::new(1), "lab", [entry]).unwrap()])
                .unwrap();
        Self {
            _dir: dir,
            table,
    ...
    fn publish(&mut self, index: usize, next: StoredRow, known: bool) {
        ...
        if known {
            self.epochs.change(&IDS, self.rows.get(index), &next);
        } else {
            self.epochs.unknown();
        }
    ...
    fn bind(&self, sql: &str) -> BoundQuery {
        Binder::new(&self.catalog, Some("lab"))
        ...
        for row in batch.selection().selected_rows() {
            rows.push(
                batch
                    .columns()
                    .iter()
                    .map(|c| c.value(row).unwrap().clone())
                    .collect(),
            );
            if rows.len() > 10000 {
                return Err("lab output ceiling".into());
            }
        }
    }
    Ok(Arc::new(rows))
...
fn flights(n: usize, clients: usize, duplicates: usize, shared: bool) {
    ...
    let expected: Vec<_> = queries
        .iter()
        .map(|q| encoded(&query(&fixture.catalog, &snapshot, q).unwrap()))
        .collect();
    ...
    std::thread::scope(|scope| {
        ...
        scope.spawn(move || {
            while !stop_ref.load(Ordering::Relaxed) {
                peak_ref.fetch_max(
                    pintail_exec::shared_memory_budget().used(),
                    Ordering::Relaxed,
                );
...
fn epochs(n: usize, ratio: usize, cached: bool) {
    ...
    let deps = dependencies(&fixture.bind(SQL)).unwrap();
    assert_eq!(deps.into_iter().collect::<Vec<_>>(), vec![1, 2, 3]);
    let mut reuse = Reuse::default();
    let mut hits = 0;
    let mut executions = 0;
    let mut query_ns = 0u64;
    let mut tracking_ns = 0u64;
    let mut ingest_ns = 0u64;
    let mut expected_time = 0u64;
    let mut relevant_updates = 0;
    let mut latency = Vec::new();
    for step in 0..41 {
        if step > 0 {
            let id = step * 13;
            if (step * 37) % 100 < ratio {
                relevant_updates += 1;
            }
            let next = make_row(
                id,
                n as u64 + step as u64 + 1,
                if (step * 37) % 100 < ratio {
                    (id % 1000) as i64 + step as i64
                } else {
                    (id % 1000) as i64
                },
                &format!("update-{step}"),
            ...
            let start = Instant::now();
            fixture.table.ingest_cdc(vec![next.clone()]).unwrap();
            ingest_ns += start.elapsed().as_nanos() as u64;
            let start = Instant::now();
            if cached {
                fixture.epochs.change(&IDS, fixture.rows.get(id), &next);
            }
            tracking_ns += start.elapsed().as_nanos() as u64;
            fixture.rows[id] = next;
        }
        let snapshot = fixture.table.snapshot();
        let start = Instant::now();
        let token = if cached {
            let bound = fixture.bind(SQL);
            let deps = dependencies(&bound).unwrap();
            Some(fixture.epochs.token(&deps))
        } else {
            None
        };
        let answer = if let Some(rows) = token.as_ref().and_then(|token| reuse.get(SQL, token)) {
            hits += 1;
            rows
        } else {
            executions += 1;
            let rows = query(&fixture.catalog, &snapshot, SQL).unwrap();
            if cached {
                reuse.put(SQL.into(), token.unwrap(), Arc::clone(&rows));
            }
            rows
        };
        let bytes = encoded(&answer);
        let elapsed = start.elapsed().as_nanos() as u64;
        query_ns += elapsed;
        ...
        assert_eq!(
            bytes,
            encoded(&query(&fixture.catalog, &snapshot, SQL).unwrap())
        );
        expected_time += start.elapsed().as_nanos() as u64;
    }
    println!(
        "{}",
        serde_json::json!({"experiment":"epochs","cached":cached,"rows":n,"relevant_percent":ratio,"actual_relevant_updates":relevant_updates,"hits":hits,"executions":executions,"query_ns":query_ns,"tracking_ns":tracking_ns,"ingest_ns":ingest_ns,"verification_ns":expected_time,"latency_ns":latency,"process_peak_rss_kib":peak_rss()})
    );
}
fn main() {
...
mod tests {
    ...
    fn binder_tracks_filter_and_hidden_order_columns_and_refuses_unknown_shapes() {
        ...
        for (sql, ids) in [
            (
                "SELECT note FROM samples WHERE metric > 3 ORDER BY bucket",
                vec![2, 3, 4],
            ),
            ("SELECT COUNT(*) FROM samples", vec![]),
            (SQL, vec![1, 2, 3]),
        ] {
            assert_eq!(
                dependencies(&f.bind(sql))
                    .unwrap()
                    .into_iter()
                    .collect::<Vec<_>>(),
                ids
        ...
        for sql in [
            "SELECT RAND() FROM samples",
            "SELECT NOW() FROM samples",
            "SELECT a.id FROM samples a JOIN samples b ON a.id=b.id",
            "SELECT metric FROM samples UNION ALL SELECT metric FROM samples",
        ] {
    ...
    fn actual_store_updates_preserve_only_proven_dependencies() {
        ...
        let deps = dependencies(&f.bind(SQL)).unwrap();
        let mut cache = Reuse::default();
        let old_snapshot = f.table.snapshot();
        let old = query(&f.catalog, &old_snapshot, SQL).unwrap();
        let old_token = f.epochs.token(&deps);
        cache.put(SQL.into(), old_token.clone(), Arc::clone(&old));
        f.publish(13, make_row(13, 100, 13, "other", false), true);
        assert_eq!(f.epochs.token(&deps), old_token);
        assert_eq!(
            *cache.get(SQL, &old_token).unwrap(),
            *query(&f.catalog, &f.table.snapshot(), SQL).unwrap()
        );
        f.publish(13, make_row(13, 101, -1, "other", false), true);
        assert!(cache.get(SQL, &f.epochs.token(&deps)).is_none());
        assert_ne!(*old, *query(&f.catalog, &f.table.snapshot(), SQL).unwrap());
        assert_eq!(*old, *query(&f.catalog, &old_snapshot, SQL).unwrap());
        let before = f.epochs.token(&deps);
        f.publish(64, make_row(64, 102, 400, "insert", false), true);
        assert_ne!(before, f.epochs.token(&deps));
        let before = f.epochs.token(&deps);
        f.publish(12, make_row(12, 103, 12, "delete", true), true);
        assert_ne!(before, f.epochs.token(&deps));
        let before = f.epochs.token(&deps);
        f.publish(14, make_row(14, 104, 14, "unknown", false), false);
        assert_ne!(before, f.epochs.token(&deps));
        f.table.flush().unwrap();
        let after_flush = query(&f.catalog, &f.table.snapshot(), SQL).unwrap();
        drop(f.table);
        let reopened = TableStore::open(f._dir.path(), schema(), StoreOptions::default()).unwrap();
        assert_eq!(
            *after_flush,
            *query(&f.catalog, &reopened.snapshot(), SQL).unwrap()
        );
    }
    #[test]
    fn old_completion_cannot_validate_new_snapshot_and_schema_changes_invalidate() {
        ...
        let old = std::thread::spawn(move || {
            go_rx.recv().unwrap();
            query(&catalog, &old_snapshot, SQL).unwrap()
        });
        f.publish(13, make_row(13, 100, -1, "relevant", false), true);
        let new_token = f.epochs.token(&deps);
        go_tx.send(()).unwrap();
        let mut cache = Reuse::default();
        cache.put(SQL.into(), old_token, old.join().unwrap());
        assert!(cache.get(SQL, &new_token).is_none());
        ...
        let new_schema = TableSchema::new(
            ...
            vec![
                ...
                Column::new(2, "bucket", DataType::UInt64, false),
                Column::new(3, "metric", DataType::Int64, true),
                Column::new(4, "annotation", DataType::Utf8, true),
            ],
        )
        .unwrap();
        f.table.evolve_schema(new_schema.clone()).unwrap();
        f.epochs.unknown();
        ...
        f.catalog =
            CatalogSnapshot::new([DatabaseEntry::new(DatabaseId::new(1), "lab", [entry]).unwrap()])
                .unwrap();
        assert_ne!(new_token, f.epochs.token(&deps));
        assert_eq!(
            *before,
            *query(&f.catalog, &f.table.snapshot(), SQL).unwrap()
        );
