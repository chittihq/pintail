...
native_factor+=['','The ratio divides the sums of the six dirty-state query times; it is not the ratio','of the medians displayed beside it.','These are single-seed, sequential SQL anchor timings between mutation batches. The','concurrent writer/cycle evidence belongs to the independently confirmed external','prototype above. The uniform and clustered regressions rule out blanket adoption.','A production rule must establish type/NULL/duplicate semantics, reserve memory and','choose based on expected fanout; none of those integration claims is made here.','']
position=lines.index('## Unresolved correctness boundary')
