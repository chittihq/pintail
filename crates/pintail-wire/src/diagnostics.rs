...
pub(super) mod tests {
    ...
    async fn date_comparisons_consume_valid_temporal_prefixes() {
        ...
        let result = backend
            .execute("SELECT d='0000-00-00',d='2005-00-00' FROM calendar_values ORDER BY d")
            .await
            .unwrap();
        assert_eq!(
            result.rows,
    ...
    #[tokio::test]
    async fn order_subqueries_read_outer_aggregate_aliases() {
        use pintail_types::Value;
        let (_directory, backend) = local_backend();
        backend
            .execute("CREATE TABLE numbers (grp INT, n INT)")
        ...
        backend
            ...
            .unwrap();
        let result = backend
            .execute("SELECT COUNT(*) AS amount FROM numbers ORDER BY (SELECT 1 HAVING amount)")
            .await
            .unwrap();
        assert_eq!(result.rows[0], vec![Value::UInt64(3)]);
        let result = backend
            .execute("SELECT COUNT(*) AS amount FROM numbers ORDER BY amount+1,amount+amount")
            .await
            .unwrap();
        assert_eq!(result.rows[0], vec![Value::UInt64(3)]);

        let result = backend.execute("SELECT grp,COUNT(*) AS amount FROM numbers GROUP BY grp ORDER BY (SELECT 1 HAVING amount>1),grp").await.unwrap();
        assert_eq!(result.rows[0], vec![Value::Int64(2), Value::UInt64(1)]);
    ...
    async fn unordered_grouped_windows_use_a_stable_group_key_traversal() {
        ...
        backend
            ...
            .unwrap();
        backend
            .execute("INSERT INTO amounts VALUES (8,8),(7,7),(6,6),(5,5),(4,4),(3,3),(2,2),(1,1)")
            .await
            .unwrap();
        let result = backend.execute("SELECT grp,ROW_NUMBER() OVER(),SUM(SUM(n)) OVER(ROWS UNBOUNDED PRECEDING) FROM amounts GROUP BY grp ORDER BY grp").await.unwrap();
        for (index, row) in result.rows.iter().enumerate() {
            let n = i64::try_from(index + 1).unwrap();
            assert_eq!(
                row,
                &vec![
                    Value::Int64(n),
