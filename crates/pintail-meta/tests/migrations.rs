...
fn opening_a_blank_control_plane_applies_the_initial_schema() {
    ...
    assert_eq!(metadata.schema_version().expect("schema version"), 23);
}
...
fn reopening_an_initialized_control_plane_is_idempotent() {
    ...
    assert_eq!(reopened.schema_version().expect("schema version"), 23);
}
...
fn version_one_control_plane_upgrades_polling_state_in_place() {
    ...
    assert_eq!(upgraded.schema_version().expect("schema version"), 23);
    drop(upgraded);
