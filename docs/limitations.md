...
| High-precision `DECIMAL` can truncate | Fixed | Precision up to 38 is stored and returned as exact canonical text, and arithmetic is exact on scaled integer units: `SUM` accumulates `i128` units, `AVG` and `/` widen by four fraction digits with half-away-from-zero rounding, and `+`/`-`/`*`, casts and literals compute on scaled units. The one remaining inexactness is chained expressions whose intermediates are division results, described under "MySQL semantic differences". |
