...
- `lc_time_names` accepts only `en_US`, `default_week_format` only `0` and
  `sql_select_limit` only `DEFAULT`; other values are refused, so `DATE_FORMAT`
  names, `WEEK()` without a mode and unlimited SELECTs always follow those
  defaults.
